...
// ─────────────────────────────────────────────────────────────────────────────
// Pipe Flow Screen — Phase 2: particles on solve, sound on rotate/solve/fail,
// achievement reporting, animated flow highlight
// ─────────────────────────────────────────────────────────────────────────────
...
fun PipeFlowScreen(hub: HubViewModel) {
    ...
    // Play fail sound when checking flow
    LaunchedEffect(state.flowResult) {
        val res = state.flowResult
        if (res != null && !res.solved) {
    ...
    Box(Modifier.fillMaxSize()) {
        Column(
            ...
            Row(Modifier.fillMaxWidth().padding(horizontal = 4.dp, vertical = 2.dp),
                ...
                Text("LVL", color = t.textSecondary, fontSize = 10.sp,
            ...
            AnimatedVisibility(result != null || state.solved) {
                ...
                Surface(Modifier.fillMaxWidth(), color = color.copy(.12f), shape = RoundedCornerShape(10.dp),
...
private fun PipeCellView(
    ...
    val borderColor by animateColorAsState(if (inFlow) t.primary else t.border, tween(200), label = "pipe_b")

    Box(
        Modifier.size(size).clip(RoundedCornerShape(5.dp)).background(t.surface)
            .border(1.dp, borderColor, RoundedCornerShape(5.dp))
            .then(if (!cell.fixed && !cell.locked) Modifier.clickable { onRotate(idx) } else Modifier),
...
private fun DrawScope.drawPipe(type: Char, lit: Boolean, t: GameTheme) {
    ...
    drawCircle(Color(0xFF3D4560), pW * 0.8f, Offset(cx, cy))
    if (lit) drawCircle(t.primary.copy(.6f), fW * 0.9f, Offset(cx, cy))
}
